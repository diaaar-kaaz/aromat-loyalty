...
3. ⚠️ **Environment — выбрать «Sandbox & Production»**, а не одно из них.
   На этом уже споткнулись 2026-08-17: ключ с одним только Production
   авторизуется на боевом хосте, но sandbox отвергает его с 403
   `BadEnvironmentKeyInToken` — то есть Debug-сборку из Xcode проверить нечем.
   Обратный выбор хуже: с sandbox-only ключом отладка идёт, а живым клиентам
   после релиза не приходит ничего.
4. **Download** — файл `AuthKey_XXXXXXXXXX.p8`
...
Если пуш не пришёл, приложение покажет причину текстом. Что они значат:
- `BadDeviceToken` — токен не из того окружения (sandbox против production).
  Приходит **после** успешной авторизации, то есть ключ в порядке.
- `BadEnvironmentKeyInToken` (403) — наоборот, отвергнут сам **ключ**: он выпущен
  без нужного окружения. Лечится только новым ключом (см. шаг 1).
- `InvalidProviderToken` — неверный Key ID / Team ID / битый `.p8`
- `TopicDisallowed` — `APNS_BUNDLE_ID` не совпадает с bundle приложения

Быстрый способ понять, чей это токен, — прогнать его по обоим хостам Apple
одним и тем же ключом и сравнить ответы. Именно так и нашли причину.

## Грабли, на которые уже наступили (2026-08-17)

**RLS в Supabase.** Новые таблицы создаются с включённым Row Level Security,
а приложение и API ходят под публичным ключом `anon`. Итог: запись падала с
500 `violates row-level security policy`, а чтение **молча** возвращало пусто —
счётчик получателей показывал 0 и выглядел как «просто никто не подписан».
Миграция теперь выключает RLS явно. Если счётчик снова застынет на нуле при
живых подписчиках — проверять это в первую очередь.

**Переменные без редеплоя не работают.** Vercel подставляет их при сборке;
добавить и не пересобрать — значит не сделать ничего.
